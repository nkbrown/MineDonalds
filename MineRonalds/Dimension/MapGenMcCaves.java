...
public class MapGenMcCaves extends MapGenBase
{
    /**
    ...
    private boolean isTopBlock(byte[] data, int index, int x, int y, int z, int chunkX, int chunkZ)
    {
        ...
        return (isExceptionBiome(biome) ? data[index] == (byte)Main.McGrass.blockID : data[index] == biome.topBlock);
    }
    ...
    {
        BiomeGenBase biome = worldObj.getBiomeGenForCoords(x + chunkX * 16, z + chunkZ * 16);
        int top    = (isExceptionBiome(biome) ? (byte)Main.McGrass.blockID : biome.topBlock);
        int filler = (isExceptionBiome(biome) ? (byte)Main.McDirt.blockID  : biome.fillerBlock);
        int block  = data[index];

        if (block == (byte)Main.McStone.blockID || block == filler || block == top)
        {
            if (y < 10)
            {
