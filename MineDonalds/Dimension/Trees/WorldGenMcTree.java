...
public class WorldGenMcTree extends WorldGenerator implements IWorldGenerator
{
	...
	public WorldGenMcTree(boolean par1)
	{
		this(par1, 8, 0, 0, false);
	}
	public WorldGenMcTree(boolean par1, int par2, int par3, int par4, boolean par5)
	{
		super(par1);
	...
	public boolean generate(World par1World, Random par2Random, int par3, int par4, int par5)
	{
		...
		if (par4 >= 1 && par4 + l + 1 <= 256)
		{
				...
				for (int l1 = par3 - b0; l1 <= par3 + b0 && flag; ++l1)
				{
					for (j1 = par5 - b0; j1 <= par5 + b0 && flag; ++j1)
					{
						if (i1 >= 0 && i1 < 256)
						{
							...
							/** Custom grass block **/
							Block block = Main.McGrass;
							if (k1 != 0 && !block.isLeaves(par1World, l1, i1, j1)

									/** Custom grass block **/
									&& k1 != Main.McGrass.blockID
									&& k1 != Main.McDirt.blockID
			...
			else
			{
				...
				/** Custom grass block **/
				Block soil = Main.McGrass;

				/** Custom Sapling block **/
				boolean isSoil = (soil != null && soil.canSustainPlant(par1World, par3, par4 - 1, par5, ForgeDirection.UP, (BlockMcSapling)Main.McSapling));
				if (isSoil && par4 < 256 - l - 1)
				{
						...
						for (j2 = par3 - i2; j2 <= par3 + i2; ++j2)
						{
							...
							for (int l2 = par5 - i2; l2 <= par5 + i2; ++l2)
							{
								...
								if (Math.abs(k2) != i2 || Math.abs(i3) != i2 || par2Random.nextInt(2) != 0 && k1 != 0)
								{
									...
									{
										/** Custom leaf block **/
										this.setBlockAndMetadata(par1World, j2, j1, l2, Main.McLeaf.blockID, this.metaLeaves);
									}
								}
							}
						}
					...
					for (j1 = 0; j1 < l; ++j1)
					{
						...
						{
							/** Custom Log block **/
							this.setBlockAndMetadata(par1World, par3, par4 + j1, par5, Main.McLog.blockID, this.metaWood);
						}
					}
			...
			IChunkProvider chunkGenerator, IChunkProvider chunkProvider)
	{
		for(int i = 0; i < 10; i++)
		{
			...
			(new WorldGenMcTree(false, 9, 0, 0, false)).generate(world, random, xCoord, yCoord, zCoord);
		}
	}
}